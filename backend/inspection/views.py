...
from inspection.models import DuplicateAttempt, Inspection
...
def attempts_view(request):
    rows = DuplicateAttempt.objects.select_related("existing").all()
    return render(request, "attempts.html", {"rows": rows})
...
def create_view(request):
    ...
    if request.method == "POST":
        try:
            ...
            duplicate = Inspection.objects.filter(
                aid_code=code, created_at__date=timezone.localdate()
            ).first()
    ...
    return render(request, "form.html", {"error": error, "duplicate": duplicate})
